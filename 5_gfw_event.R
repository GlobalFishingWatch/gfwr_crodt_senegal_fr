...
# ici on peut le remplacer par un bateau sénégalis de la liste de Paul
...
gaps |> arrange(desc(durationHours)) |> View()

# on fait une figure pour la position avec le gap le plus long

gaps_top <- gaps |> arrange(desc(durationHours)) |> slice_head(n = 1)
...
region_id <- gfw_region_id(region_name = "SEN" )
